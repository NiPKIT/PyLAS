...
from numpy import sqrt,abs,exp,pi,log,sin,cos,nan,argmin,nanargmin
...
def getFullTableAndHeaderName_hitran(TableName):
    #print('TableName=',TableName)
    fullpath_data = VARIABLES['BACKEND_DATABASE_NAME'] + '/' + TableName + '.data'
    if not os.path.isfile(fullpath_data):
        fullpath_data = VARIABLES['BACKEND_DATABASE_NAME'] + '/' + TableName + '.zip'
        if not os.path.isfile(fullpath_data):
          fullpath_data = VARIABLES['BACKEND_DATABASE_NAME'] + '/' + TableName + '.par'
          if not os.path.isfile(fullpath_data):
              raise Exception('Lonely header \"%s\"' % fullpath_data)
...
def storage2cache(TableName, wipe=True, thread=None):
    #print 'storage2cache:'
    #print('TableName',TableName)
    fullpath_data,fullpath_header = getFullTableAndHeaderName(TableName)
    ...
    if wipe or TableName not in tableList():
...
def scanForNewParfiles_hitran(StorageName):
    file_names = listdir(StorageName)
    headers = {} # without extensions!
    parfiles_without_header = []
    for file_name in file_names:
    ...
    for file_name in file_names:
        ...
        try:
            ...
            continue
        if (fext == 'par' or fext == 'zip') and fname not in headers:
            parfiles_without_header.append(fname)
    return parfiles_without_header
...
def load_data_offline(TableName, vmin, vmax, mnames, IDS, hitemp, thread, wipe=True):
    MID=[]
    for i in IDS:
        MID.append([ISO_ID[i][0],ISO_ID[i][1]])
    if wipe or TableName not in tableList():
      LOCAL_TABLE_CACHE[TableName] = {}
      LOCAL_TABLE_CACHE[TableName]['data'] = {}
      LOCAL_TABLE_CACHE[TableName]['header'] = HITRAN_DEFAULT_HEADER.copy()
      for par_name in LOCAL_TABLE_CACHE[TableName]['header']['order']:
          LOCAL_TABLE_CACHE[TableName]['data'][par_name] = []
    line_count = 0
    for molecule in mnames:
        if not thread.runs:
              break
        if hitemp and molecule in ["CO","CO2","H2O","NO","OH"]:
            path="data_hitemp"
        else:
            path="data_hitran"
        print("load "+molecule)
        file_list=[i for i in os.listdir(path+"/"+molecule) if i.endswith('.zip')]
        if len(file_list) > 1:
            wavenumbers={}
            for i in file_list:
                if not thread.runs:
                ...
                wavenumbers[i]=[float(name[3:8]),float(name[9:14])]
            if not thread.runs:
                break
            for i,j in wavenumbers.items():
                if not thread.runs:
                    break
                if j[0]<=vmin<=j[1]:
                    lower=j[0]
            ...
            if not thread.runs:
                break
            final_files=[i for i,j in wavenumbers.items() if j[0]>=lower and j[1]<=upper]
            for j in final_files:
                if not thread.runs:
                    archive.close()
                    break
                import zipfile
                archive = zipfile.ZipFile(path+"/"+molecule+"/"+j, 'r')
                if not thread.runs:
                    archive.close()
                    break
                (object_name, ext) = os.path.splitext(os.path.basename(j))
##                InfileData = archive.read(object_name+".par").splitlines()
                InfileData = archive.open(object_name+".par")
                if not thread.runs:
                    archive.close()
                    break
                InfileData = [k.decode("utf8") for k in InfileData]
                line_numbers=array([float(line[3:15]) for line in InfileData])
                if vmin < line_numbers[0]:
                    start_index=0
                elif vmin > line_numbers[-1]:
                    continue
                else:
                    start_index=nanargmin([i if i >= 0 else nan for i in line_numbers-vmin])
                if vmax > line_numbers[-1]:
                    end_index=len(line_numbers)-1
                elif vmax < line_numbers[0]:
                    continue
                else:
                    end_index = nanargmin([i if i >= 0 else nan for i in vmax-line_numbers])
                if start_index >= end_index:
                    continue
                for num, line in enumerate(InfileData):
                    if not thread.runs:
                        archive.close()
                        break
                    else:
                      if start_index <= num <= end_index:
                          if [int(line[0:2]),int(line[2])] in MID:
                              RowObject = getRowObjectFromString(line,TableName)
                              line_count += 1
                              addRowObject(RowObject,TableName)
                archive.close()
            ...
            fullpath_data = path+"/"+molecule+"/"+molecule+".data"
            if not os.path.isfile(fullpath_data):
                fullpath_data = path+"/"+molecule+"/"+molecule+".zip"
                if not os.path.isfile(fullpath_data):
                    fullpath_data = path+"/"+molecule+"/"+molecule+".par"
                    if not os.path.isfile(fullpath_data):
                        raise Exception("No data available in: "+path+"/")
            if fullpath_data.endswith('.zip'):
                import zipfile
                archive = zipfile.ZipFile(fullpath_data, 'r')
                (object_name, ext) = os.path.splitext(os.path.basename(fullpath_data))
                InfileData = archive.open(object_name+".par")
                InfileData = [i.decode("utf8") for i in InfileData]
            else:
                InfileData = open(fullpath_data,'r')
            if not thread.runs:
                break
            ...
            if start_index >= end_index:
                continue
            if not fullpath_data.endswith('.zip'):
                InfileData = open(fullpath_data,'r')
            for num, line in enumerate(InfileData):
                if not thread.runs:
                    break
...
def queryHITRAN(TableName,iso_id_list,numin,numax,thread, wipe=True):
    #import httplib
    ...
    storage2cache(TableName, thread=thread, wipe=wipe)
    print('PROCESSED')
...
def filter_table(TableName,Conditions):
...
def moleculeID(mol_name):
    ...
    ---
    """
    return ISO_NAME[(mol_name,1)][ISO_INDEX['mol_name']]

def ID(mol_ID, iso_name):
    return ISO_Isotope[(mol_ID,iso_name)][ISO_INDEX['id']]

def IsotopeID(mol_ID, iso_name):
    """
    INPUT PARAMETERS: 
        M: HITRAN molecule number
    OUTPUT PARAMETERS: 
    ...
    ---
    """
    return ISO_Isotope[(mol_ID,iso_name)][ISO_INDEX['iso_name']]

ISO_Isotope          = {									
										
(	1,	'H2(16O)'	):	[	1,	1,	0.997317,	18.010565,	'H2O'	],
(	1,	'H2(18O)'	):	[	2,	2,	0.00199983,	20.014811,	'H2O'	],
(	1,	'H2(17O)'	):	[	3,	3,	0.000372,	19.01478,	'H2O'	],
(	1,	'HD(16O)'	):	[	4,	4,	0.00031069,	19.01674,	'H2O'	],
(	1,	'HD(18O)'	):	[	5,	5,	0.000000623,	21.020985,	'H2O'	],
(	1,	'HD(17O)'	):	[	6,	6,	0.000000116,	20.020956,	'H2O'	],
(	2,	'(12C)(16O)2'	):	[	7,	1,	0.9842,	43.98983,	'CO2'	],
(	2,	'(13C)(16O)2'	):	[	8,	2,	0.01106,	44.993185,	'CO2'	],
(	2,	'(16O)(12C)(18O)'	):	[	9,	3,	0.0039471,	45.994076,	'CO2'	],
(	2,	'(16O)(12C)(17O)'	):	[	10,	4,	0.000734,	44.994045,	'CO2'	],
(	2,	'(16O)(13C)(18O)'	):	[	11,	5,	0.00004434,	46.997431,	'CO2'	],
(	2,	'(16O)(13C)(17O)'	):	[	12,	6,	0.00000825,	45.9974,	'CO2'	],
(	2,	'(12C)(18O)2'	):	[	13,	7,	0.0000039573,	47.998322,	'CO2'	],
(	2,	'(17O)(12C)(18O)'	):	[	14,	8,	0.00000147,	46.998291,	'CO2'	],
(	2,	'(13C)(18O)2'	):	[	15,	0,	0.000000044967,	49.001675,	'CO2'	],
(	2,	'(18O)(13C)(17O)'	):	[	120,	11,	0.00000001654,	48.00165,	'CO2'	],
(	2,	'(12C)(17O)2'	):	[	121,	9,	0.0000001368,	45.998262,	'CO2'	],
(	3,	'(16O)3'	):	[	16,	1,	0.992901,	47.984745,	'O3'	],
(	3,	'(16O)(16O)(18O)'	):	[	17,	2,	0.00398194,	49.988991,	'O3'	],
(	3,	'(16O)(18O)(16O)'	):	[	18,	3,	0.00199097,	49.988991,	'O3'	],
(	3,	'(16O)(16O)(17O)'	):	[	19,	4,	0.00074,	48.98896,	'O3'	],
(	3,	'(16O)(17O)(16O)'	):	[	20,	5,	0.00037,	48.98896,	'O3'	],
(	4,	'(14N)2(16O)'	):	[	21,	1,	0.990333,	44.001062,	'N2O'	],
(	4,	'(14N)(15N)(16O)'	):	[	22,	2,	0.0036409,	44.998096,	'N2O'	],
(	4,	'(15N)(14N)(16O)'	):	[	23,	3,	0.0036409,	44.998096,	'N2O'	],
(	4,	'(14N)2(18O)'	):	[	24,	4,	0.00198582,	46.005308,	'N2O'	],
(	4,	'(14N)2(17O)'	):	[	25,	5,	0.000369,	45.005278,	'N2O'	],
(	5,	'(12C)(16O)'	):	[	26,	1,	0.98654,	27.994915,	'CO'	],
(	5,	'(13C)(16O)'	):	[	27,	2,	0.01108,	28.99827,	'CO'	],
(	5,	'(12C)(18O)'	):	[	28,	3,	0.0019782,	29.999161,	'CO'	],
(	5,	'(12C)(17O)'	):	[	29,	4,	0.000368,	28.99913,	'CO'	],
(	5,	'(13C)(18O)'	):	[	30,	5,	0.00002222,	31.002516,	'CO'	],
(	5,	'(13C)(17O)'	):	[	31,	6,	0.00000413,	30.002485,	'CO'	],
(	6,	'(12C)H4'	):	[	32,	1,	0.98827,	16.0313,	'CH4'	],
(	6,	'(13C)H4'	):	[	33,	2,	0.0111,	17.034655,	'CH4'	],
(	6,	'(12C)H3D'	):	[	34,	3,	0.00061575,	17.037475,	'CH4'	],
(	6,	'(13C)H3D'	):	[	35,	4,	0.0000049203,	18.04083,	'CH4'	],
(	7,	'(16O)2'	):	[	36,	1,	0.995262,	31.98983,	'O2'	],
(	7,	'(16O)(18O)'	):	[	37,	2,	0.00399141,	33.994076,	'O2'	],
(	7,	'(16O)(17O)'	):	[	38,	3,	0.000742,	32.994045,	'O2'	],
(	8,	'(14N)(16O)'	):	[	39,	1,	0.993974,	29.997989,	'NO'	],
(	8,	'(15N)(16O)'	):	[	40,	2,	0.0036543,	30.995023,	'NO'	],
(	8,	'(14N)(18O)'	):	[	41,	3,	0.00199312,	32.002234,	'NO'	],
(	9,	'(32S)(16O)2'	):	[	42,	1,	0.94568,	63.961901,	'SO2'	],
(	9,	'(34S)(16O)2'	):	[	43,	2,	0.04195,	65.957695,	'SO2'	],
(	10,	'(14N)(16O)2'	):	[	44,	1,	0.991616,	45.992904,	'NO2'	],
(	11,	'(14N)H3'	):	[	45,	1,	0.9958715,	17.026549,	'NH3'	],
(	11,	'(15N)H3'	):	[	46,	2,	0.0036613,	18.023583,	'NH3'	],
(	12,	'H(14N)(16O)3'	):	[	47,	1,	0.98911,	62.995644,	'HNO3'	],
(	12,	'H(15N)(16O)3'	):	[	117,	2,	0.003636,	63.99268,      '	HNO3'	],
(	13,	'(16O)H'	):	[	48,	1,	0.997473,	17.00274,	'OH'	],
(	13,	'(18O)H'	):	[	49,	2,	0.00200014,	19.006986,	'OH'	],
(	13,	'(16O)D'	):	[	50,	3,	0.00015537,	18.008915,	'OH'	],
(	14,	'H(19F)'	):	[	51,	1,	0.99984425,	20.006229,	'HF'	],
(	14,	'D(19F)'	):	[	110,	2,	0.000115,	21.0125049978,	'HF'	],
(	15,	'H(35Cl)'	):	[	52,	1,	0.757587,	35.976678,	'HCl'	],
(	15,	'H(37Cl)'	):	[	53,	2,	0.242257,	37.973729,	'HCl'	],
(	15,	'D(35Cl)'	):	[	107,	3,	0.000118005,	36.9829544578,	'HCl'	],
(	15,	'D(37Cl)'	):	[	108,	4,	0.000037735,	38.9800043678,	'HCl'	],
(	16,	'H(79Br)'	):	[	54,	1,	0.50678,	79.92616,	'HBr'	],
(	16,	'H(81Br)'	):	[	55,	2,	0.49306,	81.924115,	'HBr'	],
(	16,	'D(79Br)'	):	[	111,	3,	0.0000582935,	80.9324388778,	'HBr'	],
(	16,	'D(81Br)'	):	[	112,	4,	0.0000567065,	82.9303923778,	'HBr'	],
(	17,	'H(127I)'	):	[	56,	1,	0.99984425,	127.912297,	'HI'	],
(	17,	'D(127I)'	):	[	113,	2,	0.000115,	128.918574778,	'HI'	],
(	18,	'(35Cl)(16O)'	):	[	57,	1,	0.75591,	50.963768,	'ClO'	],
(	18,	'(37Cl)(16O)'	):	[	58,	2,	0.24172,	52.960819,	'ClO'	],
(	19,	'(16O)(12C)(32S)'	):	[	59,	1,	0.93739,	59.966986,	'OCS'	],
(	19,	'(16O)(12C)(34S)'	):	[	60,	2,	0.04158,	61.96278,	'OCS'	],
(	19,	'(16O)(13C)(32S)'	):	[	61,	3,	0.01053,	60.970341,	'OCS'	],
(	19,	'(16O)(12C)(33S)'	):	[	62,	4,	0.01053,	60.966371,	'OCS'	],
(	19,	'(18O)(12C)(32S)'	):	[	63,	5,	0.00188,	61.971231,	'OCS'	],
(	20,	'H2(12C)(16O)'	):	[	64,	1,	0.98624,	30.010565,	'H2CO'	],
(	20,	'H2(13C)(16O)'	):	[	65,	2,	0.01108,	31.01392,	'H2CO'	],
(	20,	'H2(12C)(18O)'	):	[	66,	3,	0.0019776,	32.014811,	'H2CO'	],
(	21,	'H(16O)(35Cl)'	):	[	67,	1,	0.75579,	51.971593,	'HOCl'	],
(	21,	'H(16O)(37Cl)'	):	[	68,	2,	0.24168,	53.968644,	'HOCl'	],
(	22,	'(14N)2'	):	[	69,	1,	0.9926874,	28.006147,	'N2'	],
(	22,	'(14N)(15N)'	):	[	118,	2,	0.0072535,	29.997989,	'N2'	],
(	23,	'H(12C)(14N)'	):	[	70,	1,	0.98511,	27.010899,	'HCN'	],
(	23,	'H(13C)(14N)'	):	[	71,	2,	0.01107,	28.014254,	'HCN'	],
(	23,	'H(12C)(15N)'	):	[	72,	3,	0.0036217,	28.007933,	'HCN'	],
(	24,	'(12C)H3(35Cl)'	):	[	73,	1,	0.74894,	49.992328,	'CH3Cl'	],
(	24,	'(12C)H3(37Cl)'	):	[	74,	2,	0.23949,	51.989379,	'CH3Cl'	],
(	25,	'H2(16O)2'	):	[	75,	1,	0.994952,	34.00548,	'H2O2'	],
(	26,	'(12C)2H2'	):	[	76,	1,	0.9776,	26.01565,	'C2H2'	],
(	26,	'(12C)(13C)H2'	):	[	77,	2,	0.02197,	27.019005,	'C2H2'	],
(	26,	'(12C)2HD'	):	[	105,	3,	0.00030455,	27.021825,	'C2H2'	],
(	27,	'(12C)2H6'	):	[	78,	1,	0.97699,	30.04695,	'C2H6'	],
(	27,	'(12C)H3(13C)H3'	):	[	106,	2,	0.021952611,	31.050305,	'C2H6'	],
(	28,	'(31P)H3'	):	[	79,	1,	0.99953283,	33.997238,	'PH3'	],
(	29,	'(12C)(16O)(19F)2'	):	[	80,	1,	0.98654,	65.991722,	'COF2'	],
(	29,	'(13C)(16O)(19F)2'	):	[	119,	2,	0.0110834,	66.995083,	'COF2'	],
(	31,	'H2(32S)'	):	[	81,	1,	0.94988,	33.987721,	'H2S'	],
(	31,	'H2(34S)'	):	[	82,	2,	0.04214,	35.983515,	'H2S'	],
(	31,	'H2(33S)'	):	[	83,	3,	0.007498,	34.987105,	'H2S'	],
(	32,	'H(12C)(16O)(16O)H'	):	[	84,	1,	0.983898,	46.00548,	'HCOOH'	],
(	33,	'H(16O)2'	):	[	85,	1,	0.995107,	32.997655,	'HO2'	],
(	34,	'(16O)'	):	[	86,	1,	0.997628,	15.994915,	'O'	],
(	36,	'(14N)(16O)+'	):	[	87,	1,	0.993974,	29.997989,	'NOp'	],
(	37,	'H(16O)(79Br)'	):	[	88,	1,	0.5056,	95.921076,	'HOBr'	],
(	37,	'H(16O)(81Br)'	):	[	89,	2,	0.4919,	97.919027,	'HOBr'	],
(	38,	'(12C)2H4'	):	[	90,	1,	0.9773,	28.0313,	'C2H4'	],
(	38,	'(12C)H2(13C)H2'	):	[	91,	2,	0.02196,	29.034655,	'C2H4'	],
(	39,	'(12C)H3(16O)H'	):	[	92,	1,	0.98593,	32.026215,	'CH3OH'	],
(	40,	'(12C)H3(79Br)'	):	[	93,	1,	0.5013,	93.941811,	'CH3Br'	],
(	40,	'(12C)H3(81Br)'	):	[	94,	2,	0.48766,	95.939764,	'CH3Br'	],
(	41,	'(12C)H3(12C)(14N)'	):	[	95,	1,	0.97482,	41.026549,	'CH3CN'	],
(	42,	'(12C)(19F)4'	):	[	96,	1,	0.9893,	87.993616,	'CF4'	],
(	43,	'(12C)4H2'	):	[	116,	1,	0.955998,	50.01565,	'C4H2'	],
(	44,	'H(12C)3(14N)'	):	[	109,	1,	0.9646069,	51.01089903687,	'HC3N'	],
(	45,	'H2'	):	[	103,	1,	0.999688,	2.01565,	'H2'	],
(	45,	'HD'	):	[	115,	2,	0.00022997,	3.021825,	'H2'	],
(	46,	'(12C)(32S)'	):	[	97,	1,	0.939624,	43.971036,	'CS'	],
(	46,	'(12C)(34S)'	):	[	98,	2,	0.0416817,	45.966787,	'CS'	],
(	46,	'(13C)(32S)'	):	[	99,	3,	0.0105565,	44.974368,	'CS'	],
(	46,	'(12C)(33S)'	):	[	100,	4,	0.00741668,	44.970399,	'CS'	],
(	47,	'(32S)(16O)3'	):	[	114,	1,	0.9423964,	79.95682,	'SO3'	],
(	1001,	'H'	):	[	101,	1,	None,	None,	'H'	],
(	1002,	'He'	):	[	102,	1,	None,	None,	'He'	],
(	1018,	'Ar'	):	[	104,	1,	None,	None,	'Ar'	],
...
(	'H2O',	1	):	[	1,	'H2(16O)',	0.997317,	18.010565,	1	],
(	'H2O',	2	):	[	2,	'H2(18O)',	0.00199983,	20.014811,	1	],
(	'H2O',	3	):	[	3,	'H2(17O)',	0.000372,	19.01478,	1	],
(	'H2O',	4	):	[	4,	'HD(16O)',	0.00031069,	19.01674,	1	],
(	'H2O',	5	):	[	5,	'HD(18O)',	0.000000623,	21.020985,	1	],
(	'H2O',	6	):	[	6,	'HD(17O)',	0.000000116,	20.020956,	1	],
(	'CO2',	1	):	[	7,	'(12C)(16O)2',	0.9842,	43.98983,	2	],
(	'CO2',	2	):	[	8,	'(13C)(16O)2',	0.01106,	44.993185,	2	],
(	'CO2',	3	):	[	9,	'(16O)(12C)(18O)',	0.0039471,	45.994076,	2	],
(	'CO2',	4	):	[	10,	'(16O)(12C)(17O)',	0.000734,	44.994045,	2	],
(	'CO2',	5	):	[	11,	'(16O)(13C)(18O)',	0.00004434,	46.997431,	2	],
(	'CO2',	6	):	[	12,	'(16O)(13C)(17O)',	0.00000825,	45.9974,	2	],
(	'CO2',	7	):	[	13,	'(12C)(18O)2',	0.0000039573,	47.998322,	2	],
(	'CO2',	8	):	[	14,	'(17O)(12C)(18O)',	0.00000147,	46.998291,	2	],
(	'CO2',	0	):	[	15,	'(13C)(18O)2',	0.000000044967,	49.001675,	2	],
(	'CO2',	11	):	[	120,	'(18O)(13C)(17O)',	0.00000001654,	48.00165,	2	],
(	'CO2',	9	):	[	121,	'(12C)(17O)2',	0.0000001368,	45.998262,	2	],
(	'O3',	1	):	[	16,	'(16O)3',	0.992901,	47.984745,	3	],
(	'O3',	2	):	[	17,	'(16O)(16O)(18O)',	0.00398194,	49.988991,	3	],
(	'O3',	3	):	[	18,	'(16O)(18O)(16O)',	0.00199097,	49.988991,	3	],
(	'O3',	4	):	[	19,	'(16O)(16O)(17O)',	0.00074,	48.98896,	3	],
(	'O3',	5	):	[	20,	'(16O)(17O)(16O)',	0.00037,	48.98896,	3	],
(	'N2O',	1	):	[	21,	'(14N)2(16O)',	0.990333,	44.001062,	4	],
(	'N2O',	2	):	[	22,	'(14N)(15N)(16O)',	0.0036409,	44.998096,	4	],
(	'N2O',	3	):	[	23,	'(15N)(14N)(16O)',	0.0036409,	44.998096,	4	],
(	'N2O',	4	):	[	24,	'(14N)2(18O)',	0.00198582,	46.005308,	4	],
(	'N2O',	5	):	[	25,	'(14N)2(17O)',	0.000369,	45.005278,	4	],
(	'CO',	1	):	[	26,	'(12C)(16O)',	0.98654,	27.994915,	5	],
(	'CO',	2	):	[	27,	'(13C)(16O)',	0.01108,	28.99827,	5	],
(	'CO',	3	):	[	28,	'(12C)(18O)',	0.0019782,	29.999161,	5	],
(	'CO',	4	):	[	29,	'(12C)(17O)',	0.000368,	28.99913,	5	],
(	'CO',	5	):	[	30,	'(13C)(18O)',	0.00002222,	31.002516,	5	],
(	'CO',	6	):	[	31,	'(13C)(17O)',	0.00000413,	30.002485,	5	],
(	'CH4',	1	):	[	32,	'(12C)H4',	0.98827,	16.0313,	6	],
(	'CH4',	2	):	[	33,	'(13C)H4',	0.0111,	17.034655,	6	],
(	'CH4',	3	):	[	34,	'(12C)H3D',	0.00061575,	17.037475,	6	],
(	'CH4',	4	):	[	35,	'(13C)H3D',	0.0000049203,	18.04083,	6	],
(	'O2',	1	):	[	36,	'(16O)2',	0.995262,	31.98983,	7	],
(	'O2',	2	):	[	37,	'(16O)(18O)',	0.00399141,	33.994076,	7	],
(	'O2',	3	):	[	38,	'(16O)(17O)',	0.000742,	32.994045,	7	],
(	'NO',	1	):	[	39,	'(14N)(16O)',	0.993974,	29.997989,	8	],
(	'NO',	2	):	[	40,	'(15N)(16O)',	0.0036543,	30.995023,	8	],
(	'NO',	3	):	[	41,	'(14N)(18O)',	0.00199312,	32.002234,	8	],
(	'SO2',	1	):	[	42,	'(32S)(16O)2',	0.94568,	63.961901,	9	],
(	'SO2',	2	):	[	43,	'(34S)(16O)2',	0.04195,	65.957695,	9	],
(	'NO2',	1	):	[	44,	'(14N)(16O)2',	0.991616,	45.992904,	10	],
(	'NH3',	1	):	[	45,	'(14N)H3',	0.9958715,	17.026549,	11	],
(	'NH3',	2	):	[	46,	'(15N)H3',	0.0036613,	18.023583,	11	],
(	'HNO3',	1	):	[	47,	'H(14N)(16O)3',	0.98911,	62.995644,	12	],
(	'HNO3',	2	):	[	117,	'H(15N)(16O)3',	0.003636,	63.99268,      	12	],
(	'OH',	1	):	[	48,	'(16O)H',	0.997473,	17.00274,	13	],
(	'OH',	2	):	[	49,	'(18O)H',	0.00200014,	19.006986,	13	],
(	'OH',	3	):	[	50,	'(16O)D',	0.00015537,	18.008915,	13	],
(	'HF',	1	):	[	51,	'H(19F)',	0.99984425,	20.006229,	14	],
(	'HF',	2	):	[	110,	'D(19F)',	0.000115,	21.0125049978,	14	],
(	'HCl',	1	):	[	52,	'H(35Cl)',	0.757587,	35.976678,	15	],
(	'HCl',	2	):	[	53,	'H(37Cl)',	0.242257,	37.973729,	15	],
(	'HCl',	3	):	[	107,	'D(35Cl)',	0.000118005,	36.9829544578,	15	],
(	'HCl',	4	):	[	108,	'D(37Cl)',	0.000037735,	38.9800043678,	15	],
(	'HBr',	1	):	[	54,	'H(79Br)',	0.50678,	79.92616,	16	],
(	'HBr',	2	):	[	55,	'H(81Br)',	0.49306,	81.924115,	16	],
(	'HBr',	3	):	[	111,	'D(79Br)',	0.0000582935,	80.9324388778,	16	],
(	'HBr',	4	):	[	112,	'D(81Br)',	0.0000567065,	82.9303923778,	16	],
(	'HI',	1	):	[	56,	'H(127I)',	0.99984425,	127.912297,	17	],
(	'HI',	2	):	[	113,	'D(127I)',	0.000115,	128.918574778,	17	],
(	'ClO',	1	):	[	57,	'(35Cl)(16O)',	0.75591,	50.963768,	18	],
(	'ClO',	2	):	[	58,	'(37Cl)(16O)',	0.24172,	52.960819,	18	],
(	'OCS',	1	):	[	59,	'(16O)(12C)(32S)',	0.93739,	59.966986,	19	],
(	'OCS',	2	):	[	60,	'(16O)(12C)(34S)',	0.04158,	61.96278,	19	],
(	'OCS',	3	):	[	61,	'(16O)(13C)(32S)',	0.01053,	60.970341,	19	],
(	'OCS',	4	):	[	62,	'(16O)(12C)(33S)',	0.01053,	60.966371,	19	],
(	'OCS',	5	):	[	63,	'(18O)(12C)(32S)',	0.00188,	61.971231,	19	],
(	'H2CO',	1	):	[	64,	'H2(12C)(16O)',	0.98624,	30.010565,	20	],
(	'H2CO',	2	):	[	65,	'H2(13C)(16O)',	0.01108,	31.01392,	20	],
(	'H2CO',	3	):	[	66,	'H2(12C)(18O)',	0.0019776,	32.014811,	20	],
(	'HOCl',	1	):	[	67,	'H(16O)(35Cl)',	0.75579,	51.971593,	21	],
(	'HOCl',	2	):	[	68,	'H(16O)(37Cl)',	0.24168,	53.968644,	21	],
(	'N2',	1	):	[	69,	'(14N)2',	0.9926874,	28.006147,	22	],
(	'N2',	2	):	[	118,	'(14N)(15N)',	0.0072535,	29.997989,	22	],
(	'HCN',	1	):	[	70,	'H(12C)(14N)',	0.98511,	27.010899,	23	],
(	'HCN',	2	):	[	71,	'H(13C)(14N)',	0.01107,	28.014254,	23	],
(	'HCN',	3	):	[	72,	'H(12C)(15N)',	0.0036217,	28.007933,	23	],
(	'CH3Cl',	1	):	[	73,	'(12C)H3(35Cl)',	0.74894,	49.992328,	24	],
(	'CH3Cl',	2	):	[	74,	'(12C)H3(37Cl)',	0.23949,	51.989379,	24	],
(	'H2O2',	1	):	[	75,	'H2(16O)2',	0.994952,	34.00548,	25	],
(	'C2H2',	1	):	[	76,	'(12C)2H2',	0.9776,	26.01565,	26	],
(	'C2H2',	2	):	[	77,	'(12C)(13C)H2',	0.02197,	27.019005,	26	],
(	'C2H2',	3	):	[	105,	'(12C)2HD',	0.00030455,	27.021825,	26	],
(	'C2H6',	1	):	[	78,	'(12C)2H6',	0.97699,	30.04695,	27	],
(	'C2H6',	2	):	[	106,	'(12C)H3(13C)H3',	0.021952611,	31.050305,	27	],
(	'PH3',	1	):	[	79,	'(31P)H3',	0.99953283,	33.997238,	28	],
(	'COF2',	1	):	[	80,	'(12C)(16O)(19F)2',	0.98654,	65.991722,	29	],
(	'COF2',	2	):	[	119,	'(13C)(16O)(19F)2',	0.0110834,	66.995083,	29	],
(	'H2S',	1	):	[	81,	'H2(32S)',	0.94988,	33.987721,	31	],
(	'H2S',	2	):	[	82,	'H2(34S)',	0.04214,	35.983515,	31	],
(	'H2S',	3	):	[	83,	'H2(33S)',	0.007498,	34.987105,	31	],
(	'HCOOH',	1	):	[	84,	'H(12C)(16O)(16O)H',	0.983898,	46.00548,	32	],
(	'HO2',	1	):	[	85,	'H(16O)2',	0.995107,	32.997655,	33	],
(	'O',	1	):	[	86,	'(16O)',	0.997628,	15.994915,	34	],
(	'NOp',	1	):	[	87,	'(14N)(16O)+',	0.993974,	29.997989,	36	],
(	'HOBr',	1	):	[	88,	'H(16O)(79Br)',	0.5056,	95.921076,	37	],
(	'HOBr',	2	):	[	89,	'H(16O)(81Br)',	0.4919,	97.919027,	37	],
(	'C2H4',	1	):	[	90,	'(12C)2H4',	0.9773,	28.0313,	38	],
(	'C2H4',	2	):	[	91,	'(12C)H2(13C)H2',	0.02196,	29.034655,	38	],
(	'CH3OH',	1	):	[	92,	'(12C)H3(16O)H',	0.98593,	32.026215,	39	],
(	'CH3Br',	1	):	[	93,	'(12C)H3(79Br)',	0.5013,	93.941811,	40	],
(	'CH3Br',	2	):	[	94,	'(12C)H3(81Br)',	0.48766,	95.939764,	40	],
(	'CH3CN',	1	):	[	95,	'(12C)H3(12C)(14N)',	0.97482,	41.026549,	41	],
(	'CF4',	1	):	[	96,	'(12C)(19F)4',	0.9893,	87.993616,	42	],
(	'C4H2',	1	):	[	116,	'(12C)4H2',	0.955998,	50.01565,	43	],
(	'HC3N',	1	):	[	109,	'H(12C)3(14N)',	0.9646069,	51.01089903687,	44	],
(	'H2',	1	):	[	103,	'H2',	0.999688,	2.01565,	45	],
(	'H2',	2	):	[	115,	'HD',	0.00022997,	3.021825,	45	],
(	'CS',	1	):	[	97,	'(12C)(32S)',	0.939624,	43.971036,	46	],
(	'CS',	2	):	[	98,	'(12C)(34S)',	0.0416817,	45.966787,	46	],
(	'CS',	3	):	[	99,	'(13C)(32S)',	0.0105565,	44.974368,	46	],
(	'CS',	4	):	[	100,	'(12C)(33S)',	0.00741668,	44.970399,	46	],
(	'SO3',	1	):	[	114,	'(32S)(16O)3',	0.9423964,	79.95682,	47	],
(	'H',	1	):	[	101,	'H',	None,	None,	1001	],
(	'He',	1	):	[	102,	'He',	None,	None,	1002	],
(	'Ar',	1	):	[	104,	'Ar',	None,	None,	1018	],
...
To calculate absorption spectrum, use the function f():
...
def fetch_by_ids(TableName,iso_id_list,numin,numax, thread, wipe=True):
    """
    INPUT PARAMETERS: 
    ...
    queryHITRAN(TableName,iso_id_list,numin,numax,thread, wipe=wipe)
...
def fetch(TableName,M,I,numin,numax):
    ...
    ---
    """
    queryHITRAN(TableName,[ISO[(M,I)][ISO_INDEX['id']]],numin,numax,thread)
...
def absorptionCoefficient_HT(Components=None,SourceTables=None,partitionFunction=PYTIPS,
                             ...
                             File=None, Format=None, OmegaGrid=None, Concentrations=None, thread=None, IDS=None):
    """
    INPUT PARAMETERS: 
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            if RowID and RowID%1000 == 0 and thread:
              thread.emit(thread.line_counter_signal, RowID, nline)
            ...
            if Concentrations and ISO[(MoleculeNumberDB,IsoNumberDB)][0] not in IDS:
              continue
            ...
            # XXX time?
            if Concentrations:
                LineIntensity = LineIntensity*Concentrations[molecule_name]/100
...
def absorptionCoefficient_Voigt(Components=None,SourceTables=None,partitionFunction=PYTIPS,
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            # XXX time?
            if Concentrations:
                LineIntensity = LineIntensity*Concentrations[moleculeName(MoleculeNumberDB)]/100
            BoundIndexLower = bisect(Omegas,LineCenterDB-OmegaWingF)
            BoundIndexUpper = bisect(Omegas,LineCenterDB+OmegaWingF)
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            Xsect[BoundIndexLower:BoundIndexUpper] += factor / NATURAL_ABUNDANCES[(MoleculeNumberDB,IsoNumberDB)] * \
                                                      ABUNDANCES[(MoleculeNumberDB,IsoNumberDB)] * \
                                                      LineIntensity * lineshape_vals
##            print(lineshape_vals[0]*LineIntensity)
##            print(Gamma0)
##            print(NATURAL_ABUNDANCES)
##            print(ABUNDANCES)
##            print(factor)
            if thread and  not thread.runs:
                break
...
def getStickXY(TableName, thread):
    """
